...
InputState& OpenXR::GetInputState()
{
	return xrInput;
}

bool OpenXR::Init(const char* app_name, int64_t swapchain_format) 
{
	...
	{
		LOG(INFO) << xr_exts[i].extensionName;
	...
	if (!std::any_of(use_extensions.begin(), use_extensions.end(), [](const char* ext) 
		...
		if (xrCreateDebugUtilsMessengerEXT)
			xrCreateDebugUtilsMessengerEXT(instance, &debug_info, &debugMessenger);
		...
		xrEnumerateEnvironmentBlendModes(instance, systemID, app_config_view, 1, &blend_count, &blendMode);
		...
		if (!D3DRenderer::d3d_init(requirement.adapterLuid))
			return false;
		...
		xrEnumerateViewConfigurationViews(instance, systemID, app_config_view, 0, &view_count, nullptr);
		configViews.resize(view_count, { XR_TYPE_VIEW_CONFIGURATION_VIEW });
		views.resize(view_count, { XR_TYPE_VIEW });
		xrEnumerateViewConfigurationViews(instance, systemID, app_config_view, view_count, &view_count, configViews.data());
		for (uint32_t i = 0; i < view_count; i++) 
		{
			...
			for (uint32_t i = 0; i < surface_count; i++) {
				swapchain.surfaceData[i] = D3DRenderer::d3d_make_surface_data((XrBaseInStructure&)swapchain.surfaceImages[i]);
			}
...
void OpenXR::MakeActions() 
{
	...
	xrCreateActionSet(instance, &actionset_info, &xrInput.actionSet);
	...
	action_info.countSubactionPaths = _countof(xrInput.handSubactionPath);
	action_info.subactionPaths = xrInput.handSubactionPath;
	action_info.actionType = XR_ACTION_TYPE_POSE_INPUT;
	strcpy_s(action_info.actionName, "hand_pose");
	strcpy_s(action_info.localizedActionName, "Hand Pose");
	xrCreateAction(xrInput.actionSet, &action_info, &xrInput.poseAction);
	...
	xrCreateAction(xrInput.actionSet, &action_info, &xrInput.selectAction);
	...
	xrStringToPath(instance, "/user/hand/left/input/grip/pose", &pose_path[0]);
	xrStringToPath(instance, "/user/hand/right/input/grip/pose", &pose_path[1]);
	xrStringToPath(instance, "/user/hand/left/input/select/click", &select_path[0]);
	xrStringToPath(instance, "/user/hand/right/input/select/click", &select_path[1]);
	xrStringToPath(instance, "/interaction_profiles/khr/simple_controller", &profile_path);
	XrActionSuggestedBinding bindings[] = {
		{ xrInput.poseAction,   pose_path[0]   },
		{ xrInput.poseAction,   pose_path[1]   },
		{ xrInput.selectAction, select_path[0] },
		{ xrInput.selectAction, select_path[1] }, };
	...
	xrSuggestInteractionProfileBindings(instance, &suggested_binds);

	// Create frames of reference for the pose actions
	for (int32_t i = 0; i < 2; i++) {
		XrActionSpaceCreateInfo action_space_info = { XR_TYPE_ACTION_SPACE_CREATE_INFO };
		action_space_info.action = xrInput.poseAction;
		action_space_info.poseInActionSpace = OpenXR::GetIdentityPose();
		action_space_info.subactionPath = xrInput.handSubactionPath[i];
		xrCreateActionSpace(session, &action_space_info, &xrInput.handSpace[i]);
	}

	// Attach the action set we just made to the session
	XrSessionActionSetsAttachInfo attach_info = { XR_TYPE_SESSION_ACTION_SETS_ATTACH_INFO };
	attach_info.countActionSets = 1;
	attach_info.actionSets = &xrInput.actionSet;
...
void OpenXR::Shutdown() 
{
	...
	for (int32_t i = 0; i < swapchains.size(); i++) {
		...
		D3DRenderer::d3d_swapchain_destroy(swapchains[i]);
	}
	swapchains.clear();
	...
	if (applicationSpace != XR_NULL_HANDLE) xrDestroySpace(applicationSpace);
	if (session != XR_NULL_HANDLE) xrDestroySession(session);
	if (debugMessenger != XR_NULL_HANDLE) xrDestroyDebugUtilsMessengerEXT(debugMessenger);
...
void OpenXR::PollActions() 
{
	...
	action_set.actionSet = xrInput.actionSet;
	...
	for (uint32_t hand = 0; hand < 2; hand++) {
		...
		xrGetActionStateBoolean(session, &get_info, &select_state);
		xrInput.handSelect[hand] = select_state.currentState && select_state.changedSinceLastSync;

		// If we have a select event, update the hand pose to match the event's timestamp
		if (xrInput.handSelect[hand]) {
			XrSpaceLocation space_location = { XR_TYPE_SPACE_LOCATION };
			XrResult        res = xrLocateSpace(xrInput.handSpace[hand], applicationSpace, select_state.lastChangeTime, &space_location);
			if (XR_UNQUALIFIED_SUCCESS(res) &&
				(space_location.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) != 0 &&
				(space_location.locationFlags & XR_SPACE_LOCATION_ORIENTATION_VALID_BIT) != 0) {
				xrInput.handPose[hand] = space_location.pose;
			}
		}
	}
}
...
void OpenXR::PollPredicted(XrTime predicted_time) 
{
	...
	{
		if (!xrInput.renderHand[i])
			continue;
		XrSpaceLocation spaceRelation = { XR_TYPE_SPACE_LOCATION };
		XrResult        res = xrLocateSpace(xrInput.handSpace[i], applicationSpace, predicted_time, &spaceRelation);
		if (XR_UNQUALIFIED_SUCCESS(res) &&
		...
		{
			xrInput.handPose[i] = spaceRelation.pose;
		}
	}
}
...
void OpenXR::RenderFrame() 
{
	...
	xrWaitFrame(session, nullptr, &frame_state);
	...
	// controller models.
	PollPredicted(frame_state.predictedDisplayTime);
	...
	bool session_active = sessionState == XR_SESSION_STATE_VISIBLE || sessionState == XR_SESSION_STATE_FOCUSED;
	...
	end_info.environmentBlendMode = blendMode;
...
bool OpenXR::RenderLayer(XrTime predictedTime, std::vector<XrCompositionLayerProjectionView>& layerProjectionViews, XrCompositionLayerProjection& layer) {
	...
	locate_info.space = applicationSpace;
	xrLocateViews(session, &locate_info, &view_state, (uint32_t)views.size(), &view_count, views.data());
	layerProjectionViews.resize(view_count);
	...
	for (uint32_t i = 0; i < view_count; i++) {
		...
		xrAcquireSwapchainImage(swapchains[i].handle, &acquire_info, &img_id);

		// Wait until the image is available to render to. The compositor could still be
		// reading from it.
		XrSwapchainImageWaitInfo wait_info = { XR_TYPE_SWAPCHAIN_IMAGE_WAIT_INFO };
		wait_info.timeout = XR_INFINITE_DURATION;
		xrWaitSwapchainImage(swapchains[i].handle, &wait_info);
		...
		D3DRenderer::d3d_render_layer(layerProjectionViews[i], swapchains[i].surfaceData[img_id]);

		// And tell OpenXR we're done with rendering to this one!
		XrSwapchainImageReleaseInfo release_info = { XR_TYPE_SWAPCHAIN_IMAGE_RELEASE_INFO };
		xrReleaseSwapchainImage(swapchains[i].handle, &release_info);
	}
